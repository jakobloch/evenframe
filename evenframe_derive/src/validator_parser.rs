use helpers::evenframe::validator::Validator;
use quote::quote;
...
fn suggest_validator_correction(expr_str: &str) -> String {
    ...
    let suggestions = vec![
        ...
        ("min", "NumberValidator::Min(n) or StringValidator::MinLength(n)"),
        ("max", "NumberValidator::Max(n) or StringValidator::MaxLength(n)"),
        ...
        ("required", "This is typically handled by Option<T> types, not validators"),
    ];
