...
fn parse_generic_args(ty: &Type, type_name: &str, args: &syn::punctuated::Punctuated<GenericArgument, syn::token::Comma>) -> proc_macro2::TokenStream {
    match (type_name, args.len()) {
        ("Option" | "Vec" | "RecordLink", 1) => {
            if let Some(GenericArgument::Type(inner_ty)) = args.first() {
                ...
                match type_name {
                    "Option" => quote! { ::helpers::evenframe::schemasync::FieldType::Option(Box::new(#inner_parsed)) },
                    "Vec" => quote! { ::helpers::evenframe::schemasync::FieldType::Vec(Box::new(#inner_parsed)) },
                    "RecordLink" => quote! { ::helpers::evenframe::schemasync::FieldType::RecordLink(Box::new(#inner_parsed)) },
                    _ => unreachable!(),
                }
            } else {
                syn::Error::new(
        ...
        ("HashMap" | "BTreeMap", 2) => {
            ...
            match (args_iter.next(), args_iter.next()) {
                (Some(GenericArgument::Type(key_ty)), Some(GenericArgument::Type(value_ty))) => {
                    ...
                    match type_name {
                        "HashMap" => quote! { ::helpers::evenframe::schemasync::FieldType::HashMap(Box::new(#key_parsed), Box::new(#value_parsed)) },
                        "BTreeMap" => quote! { ::helpers::evenframe::schemasync::FieldType::BTreeMap(Box::new(#key_parsed), Box::new(#value_parsed)) },
                        _ => unreachable!(),
                    }
                },
                _ => syn::Error::new(
        ...
        (name, count) => {
            let expected = match name {
                "Option" | "Vec" | "RecordLink" => 1,
                "HashMap" | "BTreeMap" => 2,
                _ => return unsupported_type_error(ty, &format!("{}<...>", name), "Unknown generic type"),
            };
            syn::Error::new(
                args.span(),
                format!("{} must have exactly {} type parameter{}, found {}", 
                    name, expected, if expected == 1 { "" } else { "s" }, count)
            ).to_compile_error()
        }
    }
}
...
fn parse_simple_type(name: &str) -> Option<proc_macro2::TokenStream> {
    match name {
        "String" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::String }),
        "char" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::Char }),
        "bool" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::Bool }),
        "f32" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::F32 }),
        "f64" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::F64 }),
        "i8" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::I8 }),
        "i16" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::I16 }),
        "i32" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::I32 }),
        "i64" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::I64 }),
        "i128" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::I128 }),
        "isize" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::Isize }),
        "u8" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::U8 }),
        "u16" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::U16 }),
        "u32" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::U32 }),
        "u64" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::U64 }),
        "u128" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::U128 }),
        "usize" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::Usize }),
        "EvenframeRecordId" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::EvenframeRecordId }),
        "Decimal" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::Decimal }),
        "DateTime" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::DateTime }),
        "Duration" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::Duration }),
        "Tz" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::Timezone }),
        "()" => Some(quote! { ::helpers::evenframe::schemasync::FieldType::Unit }),
        _ => None,
    }
}
...
fn parse_path_type(ty: &Type, type_path: &syn::TypePath) -> proc_macro2::TokenStream {
    ...
    if let Some(last_segment) = type_path.path.segments.last() {
        ...
        match ident_str.as_str() {
            "DateTime" => return quote! { ::helpers::evenframe::schemasync::FieldType::DateTime },
            "Duration" => return quote! { ::helpers::evenframe::schemasync::FieldType::Duration },
            "Decimal" => return quote! { ::helpers::evenframe::schemasync::FieldType::Decimal },
            "Tz" => return quote! { ::helpers::evenframe::schemasync::FieldType::Timezone },
            _ => {}
        }
